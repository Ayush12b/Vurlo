...
function OrderSuccessContent({ orderId }: { orderId?: string }) {
    ...
    return (
      <div className="relative mx-auto max-w-md px-6 py-24 text-center flex flex-col items-center justify-center space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-white/40 mt-1">You haven't placed any orders yet.</p>
        </div>
        <Link
          to="/"
  ...
  const date = latestOrder.createdAt
    ? new Date(latestOrder.createdAt.seconds * 1000).toLocaleDateString("en-IN", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  return (
    <div className="relative mx-auto max-w-2xl px-6 py-16 sm:py-24 flex flex-col items-center">
      ...
      {/* Success Badge / Icon */}
      <div className="mb-6 flex items-center justify-center w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.15)] animate-bounce">
      ...
      <div className="text-center mb-10 space-y-2">
        ...
        <p className="text-sm text-white/45 max-w-md mx-auto">
          Thank you for choosing VURLO. Your payment was secure, and your premium tech is preparing
          for shipment.
        </p>
      </div>
